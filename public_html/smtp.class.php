...
function send_mail( $mailfrom, $namefrom, $mailto, $subject, $message, $xheader = "", $attname = "", $attype = "application/octet-stream", $atdata = "", $html = "")
{
    $mail = new phpmailer;
    ...
    if ( $xheader) $mail->AddCustomHeader( $xheader);
    if ( $attname) $mail->AddAttachment( $path = "", $attname, $attype, $atdata);

    if ( $html) $mail->IsHTML( $html);
    ...
    if ( ( $resultmail = $mail->Send( )) === false) {
        $result = $mail->ErrorAlerts[count( $mail->ErrorAlerts)-1];
    } else {
        $result = "sent";
    } 
...
class phpmailer {
    ...
    function create_header( )
    {
        ...
        $this->Start( );
        $header = array( );
        $header[] = sprintf( "Received: from client %s for SMTPMailClient (webmail client); %s %s\r\n", $this->IPAddress, date( "D, j M Y G:i:s"), $this->timezone);
    ...
    // ATTACHMENT METHODS
    // ///////////////////////////////////////////////
    function AddAttachment( $path, $name = "", $type = "application/octet-stream", $data = "") // $data="" jrpi
    {
        if ( !@is_file( $path) and !$data) {
        ...
        $this->attachment[$cur][4] = $data; #jrpi
        return true;
    } 
    ...
    function attach_all( )
    {
        ...
        for( $i = 0; $i < count( $this->attachment); $i++) {
            ...
            $adata = $this->attachment[$i][4]; # jrpi
            $mime .= sprintf( "----=%s\r\n", $this->boundary);
            ...
            if ( $adata) {
                ...
                if ( !$mime .= sprintf( "%s\r\n\r\n", $this->encode_file( $path))) return false;
            } 
        } 
        $mime .= sprintf( "\r\n----=%s--\r\n", $this->boundary);
...
class SMTP {
    ...
    function Data( $msg_data)
    {
        ...
        // headers.
        $in_headers = false;
        if ( preg_match( "/[A-Za-z0-9_-]+:/", $lines[0])) $in_headers = true;

        $max_line_length = 998; # used below; set here for ease in change
        $lines_out = array( );
        foreach( $lines as $line) {
            if ( empty( $line) && $in_headers) {
                $in_headers = false;
            } 
