...
if ( $xtag != '' && $ID != '') {
    ...
    if ( $del == 1 && $mid > 0) {
        if ( !isset( $_SESSION['read_only']) || $_SESSION["my_rec"] == $ID) {
            $sql = "delete from ! where tree= ? and tag='NREF' and hid = ?";
            $r1 = $db->query( $sql, array( $db->gedcomTable, $user, $mid));
            if ( $r1 != false) {
                $mydata['delmsg'] = $db->mytrans( '##Note # $mid has been deleted successfully.##');
            } else {
                $mydata['delmsg'] = $db->mytrans( "##Note delete problem.##");
            } 
        } 
    } elseif ( $add == 1) {
        if ( !isset( $_SESSION['read_only']) || $_SESSION["my_rec"] == $ID) {
            $maxid = $db->getMaxNoteID( ) + 1;

            $sql = "insert into ! (id, type,tree, tag, level, data) values(?, 'N', ?,  'NOTE', '0', ?)";
            $db->query( $sql, array( $db->gedcomTable, $maxid, $user, "@N" . $maxid . "@"));
            ...
            if ( $ima == 0) {
                $txt = preg_replace( "/1 " . strtoupper( $xtag) . "/", "1 " . strtoupper( $xtag) . CRLF . "2 EVEN" . CRLF . "3 NREF @N" . $maxid . "@", $txt, 1);
            } else {
                $l2 = split( CRLF, $txt);
                $m = "";
                for ( $i = 0; $i < count( $l2); $i++) {
                    if ( $i == $cline) {
                        $m .= $l2[$i] . CRLF . "3 NREF @N" . $maxid . "@" . CRLF;
                    } else
                        $m .= $l2[$i] . CRLF;
                } 
